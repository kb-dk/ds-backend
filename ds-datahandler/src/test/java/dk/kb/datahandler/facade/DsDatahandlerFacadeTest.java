...
public class DsDatahandlerFacadeTest extends TestcontainersUtil {
        ...
        try (MockedStatic<DsDatahandlerFacade> mockedDsDatahandlerFacade = mockStatic(DsDatahandlerFacade.class, Mockito.CALLS_REAL_METHODS)) {
            ...
            try (MockedConstruction<RerunClusterStorage> mockedConstruction = Mockito.mockConstruction(
                RerunClusterStorage.class, (mock, context) ->
                    Mockito.when(mock.getRerunClusters(any())).thenReturn(rerunClusterDtoList))) {

                    Mockito.when(dsStorageClient.latestCreated()).thenReturn(createdDto);
                    Mockito.when(dsStorageClient.updateRerunClusters(rerunClusterDtoList)).thenReturn(recordsCountDto);
    ...
    public void startJob_whenThereIsAlreadyRunningJobWithTheSameName_thenThrowInvalidArgumentServiceException() {
        ...
        String expectedMessage = "dk.kb.util.webservice.exception.InvalidArgumentServiceException: dk.kb.util.webservice.exception.InvalidArgumentServiceException: There is already a/an kaltura upload job running";
