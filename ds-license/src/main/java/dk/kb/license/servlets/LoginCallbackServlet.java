...
public class LoginCallbackServlet extends HttpServlet {
    ...
    /**
     * After login at KeyCloak users are redirected to this servlet.
     * Login URL at KeyCloak: https://<keycloak_server>realms/master/protocol/openid-connect/auth?response_type=code&client_id=kb-ad&redirect_uri=http://<redirect-server>/ds-license/loginCallbackServlet
     * After successful login, users are redirected back to this servlet.
     * Example redirect url: http://<redirect-server>/ds-license/loginCallbackServlet?session_state=96bff8da-63d4-4f61-850d-23317ece7f90&iss=https%3A%2F%2F<keycloak_server>%2Frealms%2Fmaster&code=b6374b3d-b182-4e9f-acd1-20cf00a2c0d2.96bff8da-63d4-4f61-850d-23317ece7f90.34931351-1c06-48e8-a120-36ae04e1bff1
     * The url parameter 'code' and keycloak secret (from property file) are used to fetch user data from keycloak and validate redirect came from keycloak.
     * The response from keycloak is JWT encoded. The value user name and user email are extracted from the JWT and set on session to mark user is logged in.
     */
    @Override
