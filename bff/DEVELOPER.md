...
After a fresh checkout or after the `bff-openapi_v1.yaml` specification has changed, the `api` and the `model` files 
...
<http://localhost:8080/bff/v1/hello>
where "bff" is your artifactID from above.

The Swagger UI is available at <http://localhost:8080/bff/api/>, providing access to the `v1` version of the GUI. 
...
Access to the merged configuration is through the static class at `src/main/java/dk.kb.oauth/config/ServiceConfig.java`.
...
* `conf/bff-base.yaml`: Application core config, such as limits for input parameters, rules
for transformations and thumbnail sizes  
  _Shared by everyone, controlled by developers, part of the repo._
* `conf/bff-environment.yaml.sample`: Sample/template for environment config, such as server names, 
...
* `conf/bff-local.yaml`: Concrete environment config for the individual developer, mirroring   
`conf/bff-environment.yaml.sample` but with locally working values. When needed for experimental
...
* `conf/bff-logback.xml`: File-oriented Logback config, intended for deploy in Tomcat.
...
repositories! To guard against this, `conf/bff-environment.yaml` is added to `.gitignore`. 
...
Everything is defined centrally in the file [src/main/openapi/bff-openapi_v1.yaml](src/main/openapi/bff-openapi_v1.yaml).
IntelliJ IDEA has a plugin for editing OpenAPI files that provides a semi-live preview of the generated GUI and
the online [Swagger Editor](https://editor.swagger.io/) can be used by copy-pasting the content of `bff-openapi_v1.yaml`.
...
A common pattern during initial definition of the `bff-openapi_v1.yaml` is to delay the implementation phase and recreate 
the skeleton implementation files on each build. This can be done by setting `generateOperationBody` in the `pom.xml` 
to `true`.

**Tip:** If the `bff-openapi_v1.yaml` is changed a lot during later development of the application, it might be better to have 
`<generateOperationBody>true</generateOperationBody>` in `pom.xml` and add the implementation code to manually created
classed (initially copied from the OpenAPI-generated skeleton impl classes). When changes to `bff-openapi_v1.yaml` results in
...
should
  ...
  * Create a new OpenAPI YAML, e.g. `bff-openapi_v2.yaml` (preferably by copying `bff-openapi_v1.yaml` and adjusting) and place
    it alongside `bff-openapi_v1.yaml`
    * Edit the old `bff-openapi_v1.yaml` and add a note that a new version is available
  ...
  * Add the path to the new OpenAPI YAML to `urls` in `webapp/api/index.html`. The first entry in the array is the
    default when visiting the main API-page at <http://localhost:8080/bff/api/>  
...
```
[Unreleased](https://github.com/kb-dk/bff-template/compare/v1.0.0...HEAD)
[1.1.0](https://github.com/kb-dk/bff-template/compare/v1.0.0...v1.1.0)
[1.0.0](https://github.com/kb-dk/bff-template/releases/tag/v1.0.0)
```

For BitBucket (KB's internal git), the syntax is
```
[Unreleased](https://sbprojects.statsbiblioteket.dk/stash/projects/ARK/repos/bff-template/compare/commits?targetBranch=refs%2Ftags%2Fv1.1.0&sourceBranch=refs%2Fheads%2Fmaster)
[1.1.0](https://sbprojects.statsbiblioteket.dk/stash/projects/ARK/repos/bff-template/compare/commits?targetBranch=refs%2Ftags%2Fv1.0.0&sourceBranch=refs%2Ftags%2Fv1.1.0)
[1.0.0](https://sbprojects.statsbiblioteket.dk/stash/projects/ARK/repos/bff-template/commits?until=refs%2Ftags%2Fv1.0.0)
```
Note that both the `ARK`-part and the repo-id `bff-template` is project-specific.


## Release procedure

1. Review that the `version` in `pom.xml` is fitting. `bff` uses
...
which boils down to
   ...
   * Run `git push`   
   ...
   `mvn release:rollback ; git tag -d bff-1.4.2 ; git push --delete origin bff-1.4.2`
